...
- Exposes `getSavedCurrenciesOrFetch()` for extension-style lookups (notably the widget configuration picker): it returns the persisted snapshot when present, otherwise does one live `getCurrencies()` fetch, whose successful result is persisted so later intent lookups stay fast
...
Widget configuration picker:
  getSavedCurrenciesOrFetch()                  // saved snapshot first; fresh installs fetch once and persist
```
